...
unsigned long GraphSimplification::removeTips()
{
    ...
    dispatcher.iterate (itNode, [&] (Node& node)
    {
        ...
        if ((inDegree == 0 || outDegree == 0) && (inDegree != 0 || outDegree != 0))
        {
            ...
            /* TODO: check why we only follow the first neighbor, not all of them (in exotic cases) */
            ...
            if (neighbors.size() > 1)
            {
                cout << endl << "neighbors of: " << _graph.toString (node.kmer) << " are this many: " << neighbors.size() << endl;
            }
            ...
            if (neighbors.size() > 1)
            {
                cout << endl << "the last node of the simple path is now : " << _graph.toString (nodes.back().kmer) << "is it the original node??" << endl;
            }
...
// moved to GraphSimplificationAttempts as we're not using it anymore
...
{
    unsigned int k = _graph.getKmerSize();
    unsigned int maxECLength = (unsigned int)((float)k * (10 - 1.0)) * 3   ;  // SPAdes mode
      ...
      TIME(auto start_thread_t=get_wtime());
        ...
        if (_graph.outdegree(node) >= 2)
        {
            ...
            if (isTopologicalEC)
            {
                ...
                // explore the other two or more simple paths connected to that node to get a abundance estimate
                Graph::Vector<Edge> neighbors = _graph.neighbors<Edge>(nodes.back());

#if 0

                // but first, get the branching node(s) the EC is connected to 
                Graph::Vector<Node> connectedBranchingNodes = _graph.neighbors<Node>(nodes.back(), neighbors[0].direction);
                unsigned int nbBranchingNodes = 0;
                double meanNeighborsCoverage = 0;
                for (size_t j = 0; j < connectedBranchingNodes.size(); j++)
                ...
                bool isEC = true; // TODO maybe: (mean_abundance < local_min_abundance); but in fact, we didn't use that in legacyTraversal. maybe it's a mistake.
